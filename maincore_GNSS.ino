...
static void print_pos(SpNavData *pNavData)
{
  ...
  else
  {
    ...
    my_lat = pNavData->latitude * 1000000;
    Serial.print(", Lon=");
    Serial.print(pNavData->longitude, 6);
    my_lon = pNavData->longitude * 1000000;
  }
...
//  my_lon = 139090938;
  ...
  MP.Recv(&msgid2, &msgdata2, 2);
  MP.Recv(&msgid3, &msgdata3, 3);

  if (counter == 10){
    Serial.println("1: SEND INIT SIGNAL");
    MP.Send(6, 1, 1);
    Serial.println("2: WAIT FOR SIGNAL ACK");
    while(msgid4 != 15) {
      MP.Recv(&msgid4, &msgdata4, 1);
    }
    msgid4 = 0;
    Serial.println("5: ACK RECEIVED THEN SEND DATA");
    MP.Send(0, my_lat, 1);
    MP.Send(1, my_lon, 1);
    Serial.println("6: WAIT FOR DATA ACK");
    while(msgid1 != 16) {
      MP.Recv(&msgid1, &msgdata1, 1);
    }
    Serial.println("9: FINISH PROC");
    msgid1 = 0;
    
    if(msgdata2) {
      Serial.println("SEND TO SUB 2");
      MP.Send(2, my_lat, 2);
      MP.Send(3, my_lon, 2);
      msgdata2 = 0;
    }
    if(msgdata3) {
      Serial.println("SEND TO SUB 3");
      MP.Send(4, my_lat, 3);
      MP.Send(5, my_lon, 3);
      msgdata3 = 0;
    }
